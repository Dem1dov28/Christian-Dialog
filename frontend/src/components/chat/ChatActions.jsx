...
const ChatActions = ({
  ...
  isPinned = false, // Новый пропс для определения состояния закрепления
  ...
  isChannel = false,
  isModelChat = false, // Пропс для определения, является ли это чатом модели
}) => {
  ...
  const cssVars = {
    "--background-dark": "rgba(26, 39, 52, 0.9)",
  ...
  const handleAction = (callback) => (event) => {
    ...
    if (typeof callback === "function") {
      // Pass chatId, agentId, conversationId, folderId if consumer expects them
      callback({ chatId, agentId, conversationId, folderId });
    }
    closeMenu();
  };
  ...
  const getPositionStyle = () => {
    ...
    const menuHeight = 140; // Approximate menu height (4 items * ~35px each)
      ...
      <ul className="select-none divide-y divide-[var(--border-light)] dark:divide-[var(--border-dark)]">
        {isModelChat && onRenameChat && (
          <li>
            <button
              ...
              onClick={handleAction(onRenameChat)}
            >
              {React.createElement(getActionIcon("edit"), { className: "text-[var(--icon-light)] dark:text-[var(--icon-dark)] mr-2 text-lg" })}
              <span className="text-sm">{t("chat.renameChat", { defaultValue: "Изменить название" })}</span>
            </button>
          </li>
        )}
        {folderId ? (
          <li>
            ...
            >
              ...
              <span className="text-sm">
                {isPinnedInFolder
                  ? t("chat.unpinFromFolder", {
                      defaultValue: "Unpin from folder",
                    })
                  : t("chat.pinInFolder", {
                      defaultValue: "Pin in folder",
                    })}
              </span>
            </button>
          </li>
          ...
          <li>
            <button
              type="button"
              className="w-full text-left flex items-center px-3 py-2 text-[var(--text-light)] dark:text-[var(--text-dark)] hover:bg-[var(--hover-light)] dark:hover:bg-[var(--hover-dark)] transition-colors duration-150"
              onClick={handleAction(onPinToTop)}
            >
              {React.createElement(getActionIcon("pin"), { className: "text-[var(--icon-light)] dark:text-[var(--icon-dark)] mr-2 text-lg" })}
              <span className="text-sm">
                {isPinned
                  ? t("chat.unpinFromTop", {
                      defaultValue: "Unpin from top",
                    })
                  : t("chat.pinToTop", {
        ...
        <li>
          <button
            ...
            onClick={handleAction(
              isChannel && typeof onUnsubscribeChannel === "function"
                ? onUnsubscribeChannel
                : onDeleteAgent
            )}
          >
            ...
            <span className="text-sm font-medium">
              {isChannel
                ? t("modals.unsubscribeChannel.unsubscribe", {
                    defaultValue: "Отписаться",
                  })
                : t("chat.deleteChat")}
            </span>
          </button>
